...
def test_real_anchors():
    ...
    # 2026-09-11 复核：top1 已变为 健康元 sh600380（V1.64 主导），海澜之家 sh600398 退到
    # 总分 0.464（仍在本宇宙、仍过 AND 门，只是不再居首，非数据缺失）。
    # 已核对当日 11:25 的 CI 失败日志同样是 top=sh600380，早于「剔除银行/非银/地产」
    # （宇宙 780→683）落地时间，故本次变动不是该改动引入的，而是 09-09 之后
    # 某次股本/市值数据刷新的连锁重排（价值 z 对市值极敏感）。
    # ⚠️ 该锚点已第 4 次因市值口径修正而翻转（圆通→新奥→海澜之家→健康元），
    # 本质是对数据快照过拟合；再翻时应改为更稳的不变量，例如「top1 总分 > 0.9」。
    check("2026-09-07 总分 top1 = sh600380（健康元）", top == "sh600380", f"top={top}")
