...
from tests.gen_concentrated import concentrated_cfg
...
def main():
    ...
    ap.add_argument("--max-holdings", type=int, default=0,
                    help=">0 则按集中档惯例放宽单票/行业上限，只取前 N 只（默认 0 = 配置档 [20,30]）")
    args = ap.parse_args()

    cfg = load_config()
    if args.max_holdings > 0:
        cfg, mpos = concentrated_cfg(cfg, args.max_holdings)
    else:
        po = cfg.section("portfolio") or {}
        mpos = float(po.get("max_position", 0.08))
    st = Store(DB)
    ...
    lines = [
        ...
        f"- 权重来自 L6 组合构建（分数×流动性，单票≤{mpos:.0%}、"
        f"单行业≤{cfg.section('portfolio').get('max_industry', 0.25):.0%}、强制留≥10%现金）",
    ...
    tag = f"-top{args.max_holdings}" if args.max_holdings > 0 else ""
    path = OUT / f"持仓建议{tag}-{args.asof}.md"
