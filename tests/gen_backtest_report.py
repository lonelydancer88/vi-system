...
本报告把每期信息合到一期一节的叙述式报告 —— 每期给出
  ...
  ② 期末持仓（当前日期 / 权重 / 现价 / 建仓价 / 价格收益 / 持有收益(含分红)）
...
def main():
    ...
    for blk in ledger:
        ...
        snap = blk.get("snap", [])
        if snap:
            # 「当前日期」= 本期末（该调仓日）收盘时点：现价/收益都以此日收盘为准。
            # 单列出来是为了让表格脱离「第 N 期：YYYY-MM-DD」小节标题也自解释 ——
            # 否则整表复制出去会丢失时点（曾据此把 2017-11-15 的快照误当成 2018-05-15）。
            L += ["**期末持仓**", "",
            ...
            for s in sorted(snap, key=lambda x: -x["w"]):
                ...
                L.append(f"| {s['name']} | {d.date()} | {s.get('industry', '')} | {s['w']:.1%} | "
