...
- ⚠️ **改动评估的通用陷阱：剔除弱势行业会同时抬高基准。** 剔除 银行/非银/地产后
  等权基准自身 14.23%→**15.62%（+1.39pp）**，而策略只涨 0.26~0.74pp → 超额反被抹平
  （30只 +1.76%→+0.75%、Top5 +1.1%→−0.03%）。**任何宇宙/行业层面的改动，
  必须看"相对等权的超额"而非"绝对年化"**，否则会把基准改善误读成策略改善。
