...
- **宇宙**：沪深 300 + 中证 500 + 中证 1000 三池，共 **780 只**（截至 2026-09）。
...
|------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **Top30** | +11.7% | **+8.5%** | +179.4% | −13.07% | 0.98 | 64.3% | 29.1 |
| **Top5** | **+13.9%** | **+10.7%** ✅ | +235.7% | −17.00% | 0.91 | **64.3%** | 5.0 |
| **Top3** | +12.1% | **+8.9%** | +189.0% | −27.44% | 0.70 | 57.1% | 3.0 |
| 沪深300 | +3.2% | — | +34.6% | −38.2% | — | — | — |
...
五条净值曲线对比见 [`out/nav-curve.png`](out/nav-curve.png)（30只 / 5只 / 3只 / 等权全市场 / 沪深300）。

**关键结论**（以沪深300 为基准）

1. **三档年化均大幅跑赢沪深300**：超额 +8.5% ~ +10.7%，其中 **Top5 最突出（年化 +13.9%、对沪深300 超额 +10.7%、胜率 64.3%）**。
   分散到 30 只时收益降至 +11.7%，过度集中到 3 只则回撤扩大到 −27%，但三档都明显优于沪深300。
2. **回撤全面优于沪深300**：三档最大回撤 −13% ~ −27%，均远小于沪深300 的 −38.2%，
   印证了它作为"更低波动 / 更低回撤"组合的定位。
3. **价值因子的周期性**：熊市/去杠杆阶段体现防御性；成长牛与 AI 行情中持续逆风（与设计预期一致）。
4. **排雷层有效**：对 7 只历史造假/退市案例，在爆雷前明确拦下 4 只（详见第七节）。
5. **估值 verdict 已回灌组合构建**（rules v1.3）：`verdict == 已到卖点` 直接不作为买入标的，
...
| 方案 | 实现状态 | 说明 |
|------|---------|------|
| L1 点时宇宙（含退市） | ✅ | 现扩至 780 只（沪深300+中证500+中证1000） |
...
宇宙 = 沪深300 + 中证500 + 中证1000（780 只，去重）+ 注入 7 只历史造假/退市案例。
...
| 指标 | Top30 | Top5 | Top3 | 沪深300 |
|---|---|---|---|---|
| 年化收益 | +11.7% | **+13.9%** | +12.1% | +3.2% |
| 年化超额(对沪深300) | +8.5% | **+10.7%** | +8.9% | — |
| 区间总收益 | +179.4% | +235.7% | +189.0% | +34.6% |
| 最大回撤 | −13.07% | −17.00% | −27.44% | −38.2% |
| 夏普 | 0.98 | 0.91 | 0.70 | — |
| 胜率(对沪深300) | 64.3% | **64.3%** | 57.1% | — |
| 平均持仓数 | 29.1 | 5.0 | 3.0 | — |

**诚实结论：以沪深300 为基准，三档策略均显著跑赢**（年化超额 +8.5% ~ +10.7%，胜率 57%~64%），
且回撤远小于沪深300（−13% ~ −27% vs −38.2%）。Top5 在收益与胜率上最均衡；
Top3 过度集中，回撤扩大到 −27%。价值因子的超额主要来自「熊市少跌、震荡市稳健」，
而非单边牛市进攻——它在成长牛与 AI 行情中持续逆风，熊市才体现防御性。

需注意：此处沪深300 为**价格回报**口径（不含分红），策略为后复权；若用沪深300 全收益指数，
超额会相应收窄约 2%/年，但三档仍全面跑赢。回测引擎内部的对照基准是**等权全宇宙**
（年化 +13.0%，含小盘/反转溢价，是更难超越的基准；以它为基准时三档小幅跑输，信息比率接近 0），
设计文档指定的真实基准（中证红利质量 / 红利低波）需外部数据，尚未接入。
