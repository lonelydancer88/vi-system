...
| 等权全宇宙 | +13.0% | +9.8% | +213.0% | −19.51% | 0.72 | 78.6% | ≈780 |
| 沪深300 | +3.2% | — | +34.6% | −38.2% | — | — | — |

> 等权全宇宙 = 对当期 780 只宇宙等权持有，作为「无选股、纯暴露」的对照策略；其指标由 `out/_navcache/nav_equal.csv` 的 `bench` 序列同口径（rf=0、按实测期数折算年化）实算。
...
1. **三档与等权全宇宙都跑赢沪深300**：但沪深300 是价格回报、偏大盘的弱基准——**等权全宇宙自身也跑赢它**（年化 +13.0% vs +3.2%，胜率 78.6%）。
   因此"跑赢沪深300"不直接等于选股能力，需看相对等权全宇宙的增量。
2. **策略真正的增量在相对等权全宇宙的风险压缩**：Top30 夏普 0.98（等权仅 0.72）、回撤 −13% vs −19.5%；
   收益端 Top5（+13.9%）略超等权（+13.0%），Top30/Top3 略低于等权——本质是「用部分收益换更低波动 / 更低回撤」。
3. **回撤全面优于沪深300**：三档最大回撤 −13% ~ −27%，均远小于沪深300 的 −38.2%，
...
|---|---|---|---|---|---|
| 年化收益 | +11.7% | **+13.9%** | +12.1% | +13.0% | +3.2% |
| 年化超额(对沪深300) | +8.5% | **+10.7%** | +8.9% | +9.8% | — |
| 区间总收益 | +179.4% | +235.7% | +189.0% | +213.0% | +34.6% |
| 最大回撤 | −13.07% | −17.00% | −27.44% | −19.51% | −38.2% |
| 夏普 | 0.98 | 0.91 | 0.70 | 0.72 | — |
| 胜率(对沪深300) | 64.3% | **64.3%** | 57.1% | 78.6% | — |
| 平均持仓数 | 29.1 | 5.0 | 3.0 | ≈780 | — |

**诚实结论：以沪深300 为基准，三档策略均显著跑赢**（年化超额 +8.5% ~ +10.7%，胜率 57%~64%）；
但表中「等权全宇宙」一行点明——**整个宇宙等权同样跑赢沪深300（+9.8%、胜率 78.6%）**，
沪深300 偏大盘且为价格回报，属弱基准，"跑赢它"不等于有选股能力。

真正的增量在**相对等权全宇宙的风险压缩**：Top30 夏普 0.98 vs 0.72、回撤 −13% vs −19.5%；
收益端 Top5（+13.9%）略超等权（+13.0%），Top30/Top3 略低于等权——即以部分收益换更低波动与回撤（信息比率接近 0）。
