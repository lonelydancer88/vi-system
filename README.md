...
- **真实回测**：2017-05 ~ 2026-09，28 个调仓期，后复权口径，三档集中度（默认30只 / Top5 / Top3）。
...
| 档位 | 年化收益 | 年化波动 | 年化超额(对沪深300) | 区间总收益 | 最大回撤 | 夏普 | 胜率(对沪深300) | 平均持仓 |
|------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **默认30只** | **+15.81%** | **16.05%** | **+12.57%** | +292% | **−10.03%** | **1.09** | **67.9%** | 28.9 |
| **Top5** | +16.59% | 17.90% | +13.35% | +319% | −18.37% | 1.05 | 75.0% | 5.0 |
| **Top3** | +11.84% | 20.64% | +8.60% | +184% | −25.94% | 0.73 | 67.9% | 3.0 |
| 等权全宇宙 | +14.23% | 21.82% | +10.99% | +245.2% | −20.52% | 0.72 | 75.0% | ≈780 |
| 沪深300 | +3.24% | 17.49% | — | +34.6% | −38.18% | — | — | — |

> 数据口径：三档策略取自 `out/backtest.md` / `backtest-top{5,3}.md`（相对沪深300 的对比见
> `out/backtest-hs300*.md`）；等权全宇宙与沪深300 两行为基准，按 `benchmark="equal"` /
> `"sh000300"` 单独回测实算（rf=0，年化/波动/夏普/回撤/胜率同策略口径），非从报告列抄录。
...
完整报告：[`out/回测报告-2026-09-07.md`](out/回测报告-2026-09-07.md)（默认30只）、
[`out/回测报告-top5-2026-09-07.md`](out/回测报告-top5-2026-09-07.md)、
[`out/回测报告-top3-2026-09-07.md`](out/回测报告-top3-2026-09-07.md)；
汇总视图见 [`out/report-2026-09-07.html`](out/report-2026-09-07.html)。
![净值曲线（30只 / 5只 / 3只 / 等权全市场 / 沪深300 对比）](out/nav-curve.png)

五条净值曲线：策略 30只 / 最多5只 / 最多3只、等权全市场、沪深300（价格回报）。完整逐期数据见
[`out/回测报告-2026-09-07.md`](out/回测报告-2026-09-07.md)、
[`out/回测报告-top5-2026-09-07.md`](out/回测报告-top5-2026-09-07.md)、
[`out/回测报告-top3-2026-09-07.md`](out/回测报告-top3-2026-09-07.md)。

**关键结论**（以沪深300 为基准，等权全宇宙作为对照策略）

1. **三档与等权全宇宙都跑赢沪深300**：但沪深300 是价格回报、偏大盘的弱基准——**等权全宇宙自身也跑赢它**（年化 +14.23% vs +3.24%，胜率 75.0%）。
   因此"跑赢沪深300"不直接等于选股能力，需看相对等权全宇宙的增量。
2. **策略真正的增量在相对等权全宇宙的风险压缩**：默认30只 夏普 1.09（等权仅 0.72）、波动 16.05% vs 21.82%、回撤 −10.03% vs −20.52%；
   收益端 Top5（+16.59%）与默认30只（+15.81%）均超等权（+14.23%），仅 Top3（+11.84%）低于等权。
3. **集中持股的"收益换风险"在 v1.6 出现转折**：引入 L5 卖点硬纪律（透支赢家到卖点即落袋）后，Top5 年化（16.59%）首次小幅反超 30只档（15.81%）——
   纪律性兑现收益对集中组合是正增量；但代价是回撤仍明显更深（−18.37% vs −10.03%）。Top3 仍最差（11.84% / −25.94%），
   印证"集中到 3 只只是加风险、无补偿"。
4. **回撤全面优于沪深300**：三档最大回撤 −10.4% ~ −25.9%，均远小于沪深300 的 −38.18%，
   印证了它作为"更低波动 / 更低回撤"组合的定位。
5. **价值因子的周期性**：熊市/去杠杆阶段体现防御性；成长牛与 AI 行情中持续逆风（与设计预期一致）。
6. **排雷层有效但边界明确**：对 7 只历史造假/退市案例，在爆雷前明确拦下 **3 只**（详见第九节）。
7. **估值 verdict 双向回灌组合构建**（rules v1.3 买入侧 + v1.6 卖出侧）：`verdict == 已到卖点` 既**不作为买入标的**，
   也**强制清仓已持仓**——买、卖两侧同时规避"贵到卖点"的票，杜绝"总分第一却已透支仍进/仍持"的漏洞。

**最新一期持仓建议（2026-09-07）**：宇宙 780 → 过排雷 393 → 过 AND 门 **66 只** →
组合 **30 只 + 强制留 10% 现金**；其中 **19 只已到买点**、11 只合理区间、0 只已到卖点。
见 [`out/持仓建议-2026-09-07.md`](out/持仓建议-2026-09-07.md)。
...
- **L6 宁可少持、不满仓**：z 门较严导致候选不足，默认档平均持仓 **29.3 只**（上限 30、下限 20），
  仍强制留 ≥10% 现金；门即纪律，不做放宽凑仓。注意该 10% 现金在回测中按**零收益**处理，
  相当于每年约 1.4 个点的机会成本，与换手成本（约 1.1 个点/年）同一量级。
...
|---|---|---|---|---|
| 默认30只（配置 `[20,30]`） | 30 | 8% | 25% | 7 只 |
| Top5 | 5 | 19.8% | 50% | 2 只 |
| Top3 | 3 | 33% | 50% | 1 只 |

> 默认档的 `target_size: [20,30]` 是 **lo=20 下限 + hi=30 上限**：候选不足 20 只时不为凑数放宽门槛
> （「门即纪律」），故平均持仓 29.3 只。产物中不带 `30` 后缀，以无后缀文件名承载。
> 「Top3」就是默认30只那份名单的前 3 名，不是重新筛选——总分第一的股票会同时出现在三档组合里，只是仓位不同。
...
（三档逐期回测报告，调仓动作表已内嵌价格/收益/手数/占用）、`gen_concentrated.py`
（Top5/Top3 集中组合）、`gen_position_advice.py`（今日持仓建议 + 手数）、
`gen_html_report.py`（汇总 HTML：五策略对比总览 + A/B/C 三策略分区 + 全部卡片可折叠）、
`regen_charts.py`（净值曲线 `nav-curve.png`；另有多进程拆分版 `regen_nav_curve.py`，
以 `--dump <档位>` + `--plot` 供冷缓存/长任务使用）。
...
它按固定顺序执行 12 步：预热缓存 → 回测**三档**（默认/3/5，各带沪深300对比）→ 回测报告三档 →
当前截面（vetoes 含「复核判断」列 / valuation / portfolio）→ 持仓建议 + 集中组合 →
净值曲线 → HTML 汇总。**所有产物的截面日由脚本统一推导并显式传入**，
避免各脚本默认值不一致导致「有的报告停在昨天」。

有意不纳入该脚本的产物：`backtest-split.md`（样本内外验证，研究型，手动跑
`cli backtest --split`）、`建仓清单-*.md`（价格取自实时行情、不可复现，手动跑
`gen_position_list.py`）、`trades*.md`（其价格/收益/手数/占用列已并入「回测报告」的
调仓动作表，二者底层同源 `trade_ledger`，不再单独生成）、`reasons*.md`（与回测报告
调仓动作表同源、信息零差）。
...
> 该防线形同虚设，请勿依赖它排除风险（见 `out/持仓建议-2026-09-07.md` 顶部警示）。
...
全 A 股大宇宙（每行业数十只）才是有效的回测与排雷检验场——该判据已由 780 只可投宇宙的
真实回测兑现，见**第十节**（2017-2026，28 期）。
...
|------|-------|------|------------|---------|
| 康美药业 | 2019 | ✅ 拦下 | 爆雷前 2 年（2017-05-15） | OCF/NI(5y)=0.48（纸面利润）→ 后转 Beneish |
| 康得新 | 2019 | ✅ 拦下 | 爆雷前 2 年（2017-05-15） | OCF/NI(5y)=-1.58（纸面利润）→ 后转 Beneish |
| 东方金钰 | 2019 | ✅ 拦下 | 爆雷前 2 年（2017-05-15） | OCF/NI(5y)=-4.34（纸面利润） |
| 辅仁药业 | 2019 | ❌ 未拦下 | — | 爆雷前 4 个时点均未触发（OCF/NI 1.3~1.7、Altman Z 8~9.7，指标尚属正常） |
| 乐视网 | 2017 | ⚠️ 无法验证 | — | 数据窗口外（接口仅给近 10 年） |
| 獐子岛 | 2014 | ⚠️ 无法验证 | — | 数据窗口外（爆雷早于 2016） |
| 长生生物 | 2018 | ❌ 未拦下 | — | 疫苗质量造假属经营类，非财务操纵（排雷层覆盖边界外） |

**总判定（2026-09-11 重跑）：7 只中 3 只在爆雷前被明确拦下**（康美/康得新/东方金钰），
爆雷前各时点平均拦下率 67%；2 只因接口 10 年数据窗口限制无法验证爆雷前时点；
2 只未拦下——辅仁药业的资金占用在爆雷前财务指标尚属正常（属"提前无财务征兆"型），
长生生物属经营/产品质量造假，不在财务操纵检测范围内。

> ⚠️ 口径变更说明：此前记录为「4 只拦下」（含辅仁药业，当时由 Altman Z=1.64 触发）。
> 重跑后辅仁在爆雷前各时点的 Altman Z 为 8.10~1.97，不再触发——差异来自股本校正后
> 市值口径变化影响了 Z 的市值分量。**这是口径变更导致的结果修订，不是规则退化**，
> 但结论按当前口径如实下调为 3/7，不做粉饰。

**方法学诚实声明**：本验证证明排雷层能识别「纸面利润 + 财务操纵」类爆雷（康美/康得新/东方金钰），
但（a）依赖财务数据可得性，长生式经营造假天然不在覆盖内，辅仁式"爆雷前指标正常"亦无法提前识别；
（b）接口不提供折旧/商誉/审计意见，Beneish `depi` 用中性值 1.0 兜底，非标审计规则恒跳过；
（c）市值用账面净资产代理（退市股无行情），仅影响 Altman/估值的市值分量。
这些边界已在 `tests/validate_vetoes.py` 与 `out/veto_validation.md` 中留痕。
...
> **当前复核（2026-09-11 重跑 `tests/universe_veto_stats.py`）**：样本已扩到 **805 只**
> （股本校正后的口径），2025-05-15 时点**通过 441 只（54.8%）**、拦截 364 只（45.2%）。
> 主力筛子仍是 `ocf_to_ni_5y`（128 只 / 15.9%），其后为 `beneish_m_score` 108 只、
> `leverage` 82 只、`altman_z_score` 46 只。分行业通过率：公用事业 91.4%、银行 75.9%、
> 交通运输 71.4%、钢铁 62.5%。
> ⚠️ 上面「修复后」一列是 v1.1 修复当时的快照（780 只样本），与当前值**不可直接比较**——
> 样本口径在股本校正后已变。完整数据见 [`out/universe_veto_stats.md`](out/universe_veto_stats.md)。
...
| 指标 | 默认30只 | Top5 | Top3 | 等权全宇宙 | 沪深300 |
|---|---|---|---|---|---|
| 年化收益 | **+15.81%** | +16.59% | +11.84% | +14.23% | +3.24% |
| 年化波动 | **16.05%** | 17.90% | 20.64% | 21.82% | 17.49% |
| 年化超额(对沪深300) | **+12.57%** | +13.35% | +8.60% | +10.99% | — |
| 区间总收益 | +292% | +319% | +184% | +245.2% | +34.6% |
| 最大回撤 | **−10.03%** | −18.37% | −25.94% | −20.52% | −38.18% |
| 夏普 | **1.09** | 1.05 | 0.73 | 0.72 | — |
| 胜率(对沪深300) | **67.9%** | 75.0% | 67.9% | 75.0% | — |
| 平均持仓数 | 28.9 | 5.0 | 3.0 | ≈780 | — |
| 平均换手（买卖加总/次） | 81.7% | 101.7% | 104.2% | — | — |

**诚实结论一：以沪深300 为基准，三档均显著跑赢**（年化超额 +8.42% ~ +12.74%，胜率 67.9%~75.0%）；
但表中「等权全宇宙」一行点明——**整个宇宙等权同样跑赢沪深300（+10.99%、胜率 75.0%）**，
沪深300 偏大盘且为价格回报，属弱基准，"跑赢它"不等于有选股能力。

**诚实结论二：相对等权全宇宙，真实增量仍在「风险压缩」而非「收益增强」。**
默认30只 相对等权：年化仅 **+1.58%**（信息比率 0.13、逐期胜率 50.0%），
但波动 16.05% vs 21.82%、回撤 −10.03% vs −20.52%。IR 0.13 在 9.31 年上对应 **t≈0.4** ——
统计上**不足以判定存在选股能力**；站得住的说法是"以等额收益换到显著更低的波动与回撤"。

**诚实结论三：集中持股的"收益换风险"在 v1.6 出现转折。** 引入 L5 卖点硬纪律后，
Top5 相对等权年化 **+2.36%**（IR 0.18）、首次反超 30只档（15.99% → 15.81%，16.59% vs 15.81%），
但回撤仍更深（−18.37% vs −10.03%）；Top3 仍最差（年化 −2.38% 对等权、回撤 −25.94%）。
说明"纪律性兑现透支赢家"对集中组合是正增量，但 3 只档的特质风险仍无补偿。

**诚实结论四：换手与现金是两个吃超额的大项。** 三档平均换手（买卖加总/次）82%~104%（v1.6 因 L5 卖点硬纪律较 v1.5 升约 6~9pp），按 3.01 期/年计，
年化单边约 123%~156%；在 `cost_per_side=0.5%` 下**每年成本约 1.2%**（已扣在净值里）。
另有强制 10% 现金在回测中按**零收益**处理，机会成本约 1.4%/年（以等权 14.23% 为代理）。
两项合计约 2.6%/年，与"相对等权的净超额（30只 +1.58% / Top5 +2.36% / Top3 −2.38%）"同一量级 —— 换言之，
**当前 alpha 的大部分被交易成本与现金拖累吃掉**，降换手 / 审视现金留存的优先级高于继续调因子。

价值因子的超额来自「熊市少跌、震荡市稳健」，而非单边牛市进攻；它在成长牛与 AI 行情中持续逆风，熊市才体现防御性
（分市场状态检验：熊市/去杠杆 +5.30%、红利低波牛 +2.57%、成长牛 +1.72%、AI 行情 −5.56%）。
