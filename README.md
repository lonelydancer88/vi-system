...
|------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **默认30只** | **+15.99%** | **15.37%** | **+12.74%** | +298.0% | **−10.38%** | **1.14** | **75.0%** | 29.3 |
| **Top5** | +15.33% | 17.52% | +12.09% | +277.6% | −16.91% | 1.00 | 75.0% | 5.0 |
| **Top3** | +11.66% | 20.55% | +8.42% | +179.4% | −25.94% | 0.72 | 67.9% | 3.0 |
...
2. **策略真正的增量在相对等权全宇宙的风险压缩**：默认30只 夏普 1.14（等权仅 0.72）、波动 15.37% vs 21.82%、回撤 −10.38% vs −20.52%；
   收益端默认30只（+15.99%）与 Top5（+15.33%）均超等权（+14.23%），仅 Top3（+11.66%）低于等权。
3. **集中持股没有换来收益，只放大了风险**：Top5 年化与默认30只基本持平（15.33% vs 15.99%），但回撤从 −10.38% 扩大到 −16.91%；
   Top3 收益更低（11.66%）且回撤翻倍（−25.94%）。说明总分排名在前 5 名之外已基本不含收益信息 —— 集中只是加风险，没有补偿。
...
7. **估值 verdict 已回灌组合构建**（rules v1.3）：`verdict == 已到卖点` 直接不作为买入标的，
...
|---|---|---|---|---|---|
| 年化收益 | **+15.99%** | +15.33% | +11.66% | +14.23% | +3.24% |
| 年化波动 | **15.37%** | 17.52% | 20.55% | 21.82% | 17.49% |
| 年化超额(对沪深300) | **+12.74%** | +12.09% | +8.42% | +10.99% | — |
| 区间总收益 | +298.0% | +277.6% | +179.4% | +245.2% | +34.6% |
| 最大回撤 | **−10.38%** | −16.91% | −25.94% | −20.52% | −38.18% |
| 夏普 | **1.14** | 1.00 | 0.72 | 0.72 | — |
| 胜率(对沪深300) | **75.0%** | 75.0% | 67.9% | 75.0% | — |
| 平均持仓数 | 29.3 | 5.0 | 3.0 | ≈780 | — |
| 平均换手（买卖加总/次） | 75.4% | 94.1% | 95.7% | — | — |
...
**诚实结论二：相对等权全宇宙，真实增量在「风险压缩」而非「收益增强」。**
默认30只 相对等权：年化仅 **+1.76%**（信息比率 0.14、逐期胜率 46.4%），
但波动 15.37% vs 21.82%、回撤 −10.38% vs −20.52%。IR 0.14 在 9.31 年上对应 **t≈0.4** ——
统计上**不足以判定存在选股能力**；站得住的说法是"以等额收益换到显著更低的波动与回撤"。

**诚实结论三：集中持股没有换来收益，只放大了风险。** Top5 年化（15.33%）与默认30只（15.99%）基本持平，
回撤却从 −10.38% 扩大到 −16.91%；Top3 年化更低（11.66%）、回撤翻倍（−25.94%）、夏普降到 0.72。
说明总分排名在前 5 名之外已基本不含收益信息 —— 集中纯粹加风险，没有补偿。

**诚实结论四：换手与现金是两个吃超额的大项。** 三档平均换手（买卖加总/次）75%~96%，按 3.01 期/年计，
年化单边约 113%~144%；在 `cost_per_side=0.5%` 下**每年成本约 1.13%**（已扣在净值里）。
另有强制 10% 现金在回测中按**零收益**处理，机会成本约 1.4%/年（以等权 14.23% 为代理）。
两项合计约 2.5%/年，与"相对等权的净超额 +1.76%"同一量级 —— 换言之，
